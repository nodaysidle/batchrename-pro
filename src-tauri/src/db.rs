use crate::types::{HistoryResponse, JobSummary, Settings};
...
#[cfg(test)]
pub fn run_migrations_for_test(conn: &Connection) -> SqlResult<()> {
    run_migrations(conn)
}
...
pub fn update_job_file_result(
    ...
    conn.execute(
        "UPDATE job_files
         SET status = ?1,
             transformed_path = COALESCE(?2, transformed_path),
             backup_path = COALESCE(?3, backup_path),
             error_message = ?4
         WHERE id = ?5",
        params![
            status,
...
pub fn get_history(
    ...
    let (jobs, total_count) = if let Some(q) = search {
        ...
        let rows = stmt.query_map(
            [&like, &limit.to_string(), &offset.to_string()],
            map_job_summary,
        )?;
...
pub fn get_successful_undo_records(
    conn: &Connection,
    job_id: &str,
) -> SqlResult<Vec<UndoFileRecord>> {
    let mut stmt = conn.prepare(
        "SELECT id, original_path, transformed_path, backup_path
         FROM job_files
         WHERE job_id = ?1
           AND status = 'success'
...
pub fn get_all_settings(conn: &Connection) -> SqlResult<Settings> {
    let get = |key: &str| -> Option<String> { get_setting(conn, key).ok().flatten() };

    let mut settings = Settings::default();
    if let Some(v) = get("theme") {
        settings.theme = v;
    }
    if let Some(v) = get("accent_color") {
        settings.accent_color = v;
    }
    if let Some(v) = get("default_output_dir") {
        settings.default_output_dir = Some(v);
    }
    if let Some(v) = get("max_parallel_jobs") {
        settings.max_parallel_jobs = v.parse().unwrap_or(4);
    }
    if let Some(v) = get("auto_backup") {
        settings.auto_backup = v == "true";
    }
    if let Some(v) = get("backup_retention_days") {
        settings.backup_retention_days = v.parse().unwrap_or(30);
    }
    if let Some(v) = get("file_hard_cap") {
        settings.file_hard_cap = v.parse().unwrap_or(5000);
    }
    if let Some(v) = get("last_convert_format") {
        settings.last_convert_format = Some(v);
    }
    Ok(settings)
}

pub fn insert_search_entry(
    conn: &Connection,
    job_id: &str,
    description: &str,
    file_names: &str,
