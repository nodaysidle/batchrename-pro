...
- `apply_rename` starts a job under a short DB lock, returns the job id, and streams per-file progress. Cancel skips remaining work; it does not undo an in-flight `fs::rename`. After cancel, `file_registry` must match disk for any hop that already completed (including a `.brp-tmp-*` hop).
