...
- Tauri 2 + Vite/React/TypeScript desktop app.
- Backend logic belongs in Rust commands under `src-tauri/`; no backend server, REST API, network dependency, Electron, Next.js, Prisma, or ORM.
- Desktop-first. Keep raw `rusqlite` persistence and current file-processing architecture unless explicitly refactoring.
...
2. **Unit / fast checks**
   - `npm run typecheck`
   - Rust command-level checks, if needed: `cd src-tauri && cargo test`
